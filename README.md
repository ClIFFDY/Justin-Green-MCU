...
| 混合整数运算 | 移位 / 与 / 异或 / 加 / 减 |  
  ...
  | 内层循环：（每迭代 32 条指令）|    
  |:---------------------------|  
  | 1-3   记录 RMW：LBU → ADDI → SB       DH_REC0 += 1 | 
  | 4-5   字符字段写                       DH_REC1 = 'A'  |
  | 6-8   数组 RMW：LBU → ADD → SB         DH_ARR0 += 记录值 | 
  | 9-12  函数调用 dh_func（10 条）        结果 → DH_GLOB  |
  | 13-16 混合 ALU：SLL / AND / XOR / ADD  |
  | 17-22 条件分支（两路径等长，22 条/迭代 → 确定性 K） | 
  | 23-24 内层计数 |  
    
  （分支等长，实际执行22条 + dh_func = 32条/迭代）
  
  三重循环：内层 255 × 中层 255 × 外层 **35**  
  每 5 外层打一个 `.` 进度点（`putc`，间隔调用不破坏循环体寄存器）  
  
  MIPS 计算  

  **K ≈ 72.8M 条**：32 条/迭代 × 255 × 255 × 35    
  分子 **N = K×50>>16 = 0xD90A**（"50" = 50MHz 参考时钟归一化）  
  **MIPS = N / (elapsed >> 16) = 50 × IPC**  
  除法：16 位重复减（商 < 256），结果十进制显示    
