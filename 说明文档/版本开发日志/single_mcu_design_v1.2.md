...
| SB rs | 奇数地址（bit0=1） | 单引脚模式：`gpio_mode[addr[3:1]] <= data[1:0]` |

模式编码（data[1:0]）：00=UNUSE（上电默认，驱动 0）、01=OUT（推挽输出）、10=IN（轮询输入，高阻）、11=IRQ（中断输入，高阻）。

- **方向位** `gpio_set = mode[1]`：mode[1]=0（UNUSE/OUT）驱动、mode[1]=1（IN/IRQ）高阻读入。
