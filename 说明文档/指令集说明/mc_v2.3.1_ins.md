# MCU 指令集与外设用法说明（v2.3.1 —— RTOS）

> 配套版本：MCU v2.3.1（2026-08-19，系统层/软件升级版）。**版本号第三位 = 系统层（软件）变动**：指令集、编码、外设、中断语义与 v2.3 完全一致，无任何 ISA 改动（core RTL 无变化）；本版仅系统程序 `rtos_shell` 升级为菜单式界面（主菜单 + 子菜单 + 信息视图 + RAM bar + 倒计时/LED 功能）+ 汇编器 `asm.py` 增强（`.puts` 宏、自动补 jpad、引号注释修复、UTF-8 中文编码——纯工具能力，不影响编码格式）。
> 指令集内容本版与 v2.3 相同；v2.3 的寄存器约定（r254=j）与访存语义（0x5000 读）仍是本版依据。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v2.3.md`（内核/硬件）与 `版本开发日志/single_mcu_design_v2.3.1.md`（shell UI 系统层）。
