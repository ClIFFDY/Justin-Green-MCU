# MCU 指令集与外设用法说明（v3.4.1 —— 俄罗斯方块修复 + Dhrystone 风格 MIPS 基准 + status 界面；ISA 编码无改动）

> 配套版本：MCU **v3.4.1**（2026-08-25，软件/固件版）。**ISA 编码与 v3.0/v3.1.0/v3.2.0/v3.3.0/v3.3.1/v3.4.0 完全相同（指令总数 34），指令表/opcode/压缩布局均不变**；本文件随版本重命名一份（v3.0 起归档规范）。**v3.4.1 变化在软件层 + 一处 RTL 读路径**：
> ① **俄罗斯方块 game over 修复**：check_cell 顶上方（row≥0x80，含 0xFF=-1）判 **blocked**（原误判 free → 方块可悬顶不判失败）；game_over 先 `RJAL wait_dma` 等最后一帧 DMA 发完再切菜单（否则菜单字节与帧交错 = "爆炸"乱码）；render 棋盘首行前加换行。
> ② **MIPS 基准（Dhrystone 风格）**：cmd_status 的 MIPS 改为**选项**（按 `1` 才跑，不自动）；mips_bench 重写为**混合负载**（函数调用 dh_func / 记录数组访存 / 混合 ALU / 等长分支），K≈72.8M 条，MIPS = 50×IPC ≈ **34**（50MHz 口径；66.67MHz 真实 ≈45）；计时全寄存器（timer 读 → r6-r9/r17-r20，减法除法无 RAM 回读）。
> ③ **status 界面**：删 " TASKS" 栏；加 **UPTIME 已运行时长**（调度器 `uptime_tick` 维护 32 位秒计数，5000 tick=1s，显示 `分:秒` 固定不刷屏）。
> ④ **开机动画恢复原值**：putc 打字机外层 32（~0.6ms/字符）+ anim_bar 延迟 0x06/0xFF/0xFF（~30ms/段）——之前被 sim 测试临时值调太快。
> ⑤ **RTL（timer.v 读路径）**：`bus_data_out <= cnt[]`（时序）→ `bus_data_out = cnt[]`（组合），CPU LBU 才能读到正确 timer 计数（MIPS 计时前提）。**ISA 不变**。
> 历史版本：v3.4.0（2026-08-22 RTL 时序优化 / 66.67MHz；2026-08-24 续：irq resume bubble 补偿 + RX 中断使能）；v3.3.1（扫雷/游戏子菜单/打字机/开机动画/flush_rx）；v3.3.0（I2C + RPU 寄存器映射）；v3.2.0（DMA）；v3.1.0（外设架构重构）；v3.0（LIND/SIND + WNS）。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v3.4.1.md`。
