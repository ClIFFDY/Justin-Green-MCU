...
> 配套版本：MCU v2.2（2026-08-18，中断时序修正后定版）。在 v2.1（中断栈/嵌套 + SBI + 向量迁移）基础上，**指令集、编码、地址空间全部不变**，仅修正中断时序：irq_flush 只冲刷取指级（被中断指令写回不丢）、**irq_en 授权门控**（替换 `bytmov==0`）、**IRET W+2 语义**（程序侧需 __jpad 垫层）、**抢占断点 pc-1** 修正。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v2.2.md`。
...
- **解包**：if_reg 检测到词两半 flag 均≠00 时，用 `cstall` 冻结 pc，两半各输出 1 拍；解包后的 32bit 指令 opcode 仍在 [31:26]，压缩字段经 if_reg 原位展开，decoder 无需感知压缩。**v2.2 注意**：cstall 前半拍 irq_en=0（中断被屏蔽），后半拍恢复放行。
...
共 **31 条真实指令** + 1 条伪指令。**v2.2 指令集与 v2.1 完全一致，无新增/删除**。
...
> 地址空间与 v2.1/v2.0 相同；**v2.2 只改中断授权与恢复语义，不改任何外设与地址**。
...
- **软中断 IRQ_W（0x5000）**：ISR 活动时，连续 2 条 SB 到 0x5000–0x5FFF 改写栈顶返回地址——第 1 条 `pc_addr[11:8] <= data[3:0]`（stage→OPR），第 2 条 `pc_addr[7:0] <= data`（stage→IRQ）。之后 IRET 跳改写后的目标而非原断点（可自定义 ISR 落点/任务切换）。

> **v2.2 关键语义**：

> **① 授权门控（irq_en）**：仅当 `irq_en == 2'b11`（decoder 与 if_reg 同时放行）才接受/抢占中断。**控制转移（9 条）与 cstall 拆包前半拍强制屏蔽**（irq_en=0）→ 中断只在**顺序指令（授权点）**处派发。写程序时：想要中断能打断的指令，必须是普通顺序指令；跳转/分支前的授权点之后紧跟的 W+1 不得是真实控制转移（见 ③ __jpad）。
