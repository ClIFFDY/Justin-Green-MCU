# MCU 指令集与外设用法说明（v3.4.0 —— RTL 时序优化 / 66.67MHz；ISA 编码无改动）

> 配套版本：MCU **v3.4.0**（2026-08-22，RTL 时序优化版）。**ISA 编码与 v3.0/v3.1.0/v3.2.0/v3.3.0/v3.3.1 完全相同（指令总数 34），指令表/opcode/压缩布局均不变**；本文件随版本重命名一份（v3.0 起归档规范）。**v3.4.0 变化在 RTL 时序**（ISA/外设/地址空间/软件层不变，同 v3.3.1）：
> ① **rpu 时序化中继**（原组合映射 → 流水线一级，baseline SBI 提前生效）；② **flush 机制分离**——跳转 `jmp_flush` 无条件冲刷（避免预取残留跑飞）/ 中断 `irq_flush` 精确冲刷（减少空窗）；③ **bytmov_to_pc 提前锁存**（去 decoder 译码延迟）；⑤ **pc default 修正 +1**（取指延迟 3 级）；⑥ 汇编器 **bytmov 基准 W+2→W+3**。目标 **66.67MHz（15ns 周期，余量 0.157ns）**；72MHz 不达标（总线侧违例路径）。
> v3.3.1（2026-08-22 纯软件）：① 8x8 扫雷（LIND/SIND + 帧 DMA）；② GAME 子菜单（俄罗斯方块/扫雷）；③ 打字机 putc ~5ms/字符；④ 开机动画（7 阶段加载条解锁前）；⑤ IRET 垫层移除（微测确认 W+1）；⑥ flush_rx；⑦ asm.py .puts 跨 256 页对齐。
> v3.3.0（2026-08-22 真版）：① I2C 只输出外设（i2c_out.v/应答/ACK 报错/16 深 FIFO/SCL 双相/DMA busy2/6 槽中断）；② RPU 寄存器映射（rpu.v/baseline 窗口，`raw + ((raw<253)?baseline:0)`；0xD000 写读；任务0 base0/1=0x16/2=0x26 + SBI 切；`.base`/`rK.base`；j=r253）。
> v3.2.0（2026-08-21）DMA/双端口/向量表/UART FIFO；v3.1.0（2026-08-20）外设架构重构；v3.0（2026-08-19）LIND/SIND + WNS 优化。
> **v3.4.0 时序细节**：① rpu 时序中继（opcode/inst_raw_cont/bytmov_to_pc 全锁存）；② baseline SBI 在 rpu 检测（`opcode_in==SBI && addr==0xD`）下一拍生效，不用等总线 stall；③ 跳转无条件冲刷 / 中断精确冲刷（flush_en）；⑤ pc default +1、bytmov 基准 W+3（汇编器）。
> **2026-08-24（v3.4.0 继续）**：irq resume 加 **bubble 补偿**（`resume = pcIn - 2 + bubble`，分支/跳转 refill 期断点稳定落回跳转目标，修复 '2' 洪泛）+ **RX 中断使能**（prio=2，原 0x600A=0 撤销）。ISA 编码仍无改动。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v3.4.0.md`（RTL 时序优化）。
> 编码来源：`decoder.v` / `pc.v` / `reg_f.v` / `unzipper.v` / `rpu.v` / `ins_rom.v` / `irq_controller.v` / `single_cpu_top.v` / `single_mcu_top.v` / `bus_controller.v` / `dma.v` / `i2c_out.v` / `uart_top.v` / `ram_sec_init.v` / `ram_ext_top.v` / 各外设 `.v` / `tools/asm.py`。
