# MCU 指令集与外设用法说明（v3.0.1 —— 系统层整合版，ISA 无改动）

> 配套版本：MCU v3.0.1（2026-08-20，系统层 + 软件整合版）。**ISA 与 v3.0 完全相同（指令总数 33），无改动**；本文件随版本重命名一份（v3.0 起归档规范）。指令编码/布局/外设/中断语义以 v3.0 为准，以下头部为 v3.0.1 的版本演进说明。
...
> v3.0.1（2026-08-20）系统层整合：俄罗斯方块集成 shell（GAME 独占模式保留中断）、数据区 0xB000（ram_sec_init 初始化 RAM + 程序 LIND 读）、hex 分离 2 个（ins_rom.hex + data.hex）、shell 菜单精简 credits/status/game、修 2 个上板 bug（按 0 退出、sched_body 破坏游戏 r13）。**汇编器新增 `.data`/`.db` 数据区 + `.puts` 长文本数据区化 + hex 分离输出**。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v3.0.1.md`（俄罗斯方块集成 + 数据区化 + hex 分离）与 `版本开发日志/single_mcu_design_v3.0.md`（架构重构 + LIND/SIND）。
> 编码来源：`decoder.v` / `pc.v` / `reg_f.v` / `pre_decoder.v` / `ins_rom.v` / `irq_controller.v` / `single_cpu_top.v` / `single_mcu_top.v` / `ram_sec_init.v` / 各外设 `.v` / `tools/asm.py`。
...
> 总线访问均为 2 拍：**RAM 第 1 拍置 stall**（写不 stall）；UART/timer/GPIO/IRQ_W 无 stall。LIND/SIND 与 LBU/SB 走同一总线，可访问 RAM 与外设；**LIND 读地址需锁存维持到 done 拍**（v3.0 修复）。
