...
> 配套版本：MCU v2.0（2026-08-17，上板板测通过后归档）。**架构级重构**：在 v1.4 基础上改为**词寻址 32bit 定长取指** + **压缩指令/自动打包** + **cstall 冻结 pc**。地址空间与外设**与 v1.4 完全相同**；指令语义不变，但**编码格式全部重排**（byte0 带 flag 位、bytmov 扩 16 位词单位、分支寄存器缩 4 位）。
> 设计演进、时序与调试记录见 `版本开发日志/single_mcu_design_v2.0.md`。
> 编码来源：`decoder.v` / `pc.v` / `reg_f.v` / `if_reg.v` / `ins_rom.v` / `single_mcu_top.v` / 各外设 `.v` / `tools/asm.py`。
...
打包示例（词 0 自检，见上板程序）：`ADDI r2,r0,1 | ADDI r3,r0,2` → 两压缩指令打包为词 `0x04C1_04C2`？——实际以汇编器 listing 为准（asm.py 自动打包）。若两半其一为空则退回原长。
...
> 与 v1.4 **完全相同**（v2.0 未动外设与总线解码）。以下各节沿用 v1.4 结论。
...
> 总线访问均为 2 拍：**RAM 第 1 拍置 stall**；UART/timer/GPIO 无 stall。
> 0x5000–0x7FFF / 0xC000–0xFFFF 未分配（黑洞）。

### 2.2 ram_top（0x8000–0xBFFF，16KB）

- `LBU rd, addr` / `SB rs, addr`；`ram_top.v` + 4×`ram_sec.v`，每片 4096B BRAM，**连续 4×4KB = 16KB**。
...
- **中断**：`rx_irq = FIFO 非空`（电平）→ 源 0，向量 **0xE0**（词地址）。ISR 每 `LBU r,0x2000` 弹一字节，读到空为止；可 `SB rs,0x2000` 回发。
...
- **中断分组**：pins 1–3 → GPIO1（0xA8）；pins 4–7 → GPIO2（0x88）。电平式持续触发、无 ack；pin 保持高即反复进 ISR。
- pin0 保留；非 OR：同组多 IRQ pin 同时有效只有最后生效。

### 2.6 中断与向量

| 中断源 | irq_bus[5:3] | 子源 [2:0] | 向量 | ISR 区间 | 优先级 |
|--------|--------------|-----------|------|----------|--------|
| UART RX（FIFO 非空） | 001 | 000 | 0xE0 | 0xE0–0xF7 | 中 |
| Timer | 010 | 000 | 0xC8 | 0xC8–0xDF | 低 |
| GPIO1（pins 1–3） | 010 | 001 | 0xA8 | 0xA8–0xBF | 高 |
| GPIO2（pins 4–7） | 010 | 010 | 0x88 | 0x88–0x9F | 高 |

- **向量为【词地址】**（irq_vex：UART=224=0xE0 / TIMER=200=0xC8 / GPIO1=168=0xA8 / GPIO2=136=0x88，与 v1.4 数值相同、语义从字节地址变词地址）。上板程序 `.org 0x88/0xA8/0xC8/0xE0` 对应。
- **优先级**（v2.0 用户调整）：**GPIO > UART > Timer**（single_mcu_top 的 irq_bus 后判定覆盖；v1.4 为 GPIO > Timer > UART，用户把 rx_irq 提到 timer 之前）。
