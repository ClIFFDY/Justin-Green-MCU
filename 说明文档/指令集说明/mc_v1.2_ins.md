...
| SB rs | 奇数地址（bit0=1，如 0x8001） | 单引脚模式设置：`addr[3:1]` 选 pin，`data[1:0]` 选模式 |

模式编码（data[1:0]）：

| 值 | 模式 | 引脚行为 |
|----|------|----------|
| 00 | UNUSE（上电默认） | 未使用（驱动 0） |
| 01 | OUT 推挽输出 | 驱动 `gpio_output` |
| 10 | IN 轮询输入 | 高阻，电平可经 LBU 读回 |
| 11 | IRQ 中断输入 | 高阻，电平式触发中断 |

- **方向**：mode[1]=0（UNUSE/OUT）驱动、mode[1]=1（IN/IRQ）高阻读入。
